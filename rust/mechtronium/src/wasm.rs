...
use wasmer::{
    imports, Array, Function, FunctionType, ImportObject, Instance, Module, Resolver, Val, ValType,
    Value, WasmPtr, WasmerEnv,
};

use mechtron_core::artifact::{Artifact, ArtifactRepository};
...
impl<'a> Buffers for WasmGuest {
    ...
    fn append_to_buffer(&mut self, id: i32, value: u8) {
        self.instance
            .exports
            .get_function("append_to_buffer")
            .unwrap()
            .call(&[Value::I32(id), Value::I32(value as i32)]);
    }
}
...
impl<'a> WasmGuest {
    pub fn bind_message_artifact(
        &mut self,
        artifact_file: &Artifact,
        artifact_file_contents: &str,
        ...
        self.instance
            .exports
            .get_function("bind_message_artifact")
            .unwrap()
            .call(&[
                Value::I32(artifact_file_buffer_id),
                Value::I32(artifact_file_contents_buffer_id),
            ])?;
        return Ok(());
    }
}
